...
const MultiStepForm: React.FC<MultiStepFormProps> = ({
  ...
  const getTotalSteps = () => {
    if (productType === 'mini') {
      ...
      } else if (productSlugLower === 'vinxi') {
        return 5; // 3 pasos originales + 2 páginas de preguntas específicas + 1 página final
      }
  ...
  // Helper para manejar onChange de campos "Otro" con filtrado automático de comas
  const handleOtherFieldChange = (fieldName: string, value: string) => {
    // Filtrar comas automáticamente
    const filteredValue = value.replace(/,/g, '');
    
    // Actualizar el campo con el valor filtrado
          ...
          <>
            ...
            {productType === 'mini' ? (
              <>
                ...
                {(productSlug.toLowerCase() === 'vinxi') && currentStep === 5 && renderStep3()}
              </>
            ) : (
              <>
