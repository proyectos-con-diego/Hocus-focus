...
// Importación dinámica del banner intermedio
const IntermediateBanner = dynamic(() => import('./IntermediateBanner'), { ssr: false });
...
const components = {
  ...
  block: {
    ...
    normal: ({ children }: any) => (
      <p className="text-gray-200 leading-relaxed mb-4 text-lg" data-testid="paragraph">
        {children}
      </p>
    ),
  ...
  list: {
    bullet: ({ children }: any) => (
      <ul className="list-disc list-inside text-gray-200 mb-4 space-y-2 text-lg" data-testid="bullet-list">
        {children}
      </ul>
    ),
    number: ({ children }: any) => (
      <ol className="list-decimal list-inside text-gray-200 mb-4 space-y-2 text-lg" data-testid="numbered-list">
        {children}
      </ol>
    ),
  },
  listItem: ({ children }: any) => (
    <li className="text-gray-200 text-lg" data-testid="list-item">
      {children}
    </li>
  ),
  marks: {
...
export default function ArticleContent({ post, firstHalf, secondHalf }: ArticleContentProps) {
  return (
    <article className="prose prose-xl max-w-none text-gray-100 bg-transparent prose-headings:text-white prose-h1:text-white prose-h2:text-blue-400 prose-h3:text-blue-300 prose-p:text-gray-200 prose-li:text-gray-200 prose-strong:text-white prose-blockquote:border-purple-500 prose-blockquote:bg-gray-800/50 prose-blockquote:text-gray-300" data-testid="article-body" style={{ background: 'transparent !important', border: 'none !important', borderRadius: '0 !important', padding: '0 !important' }}>
      <PortableText value={firstHalf} components={components} />
      
      {/* Banner intermedio con producto relacionado */}
      <IntermediateBanner post={post} />
      
      <PortableText value={secondHalf} components={components} />
      
      {/* Share buttons se renderizarán aquí por el componente cliente */}
    </article>
  );
}
